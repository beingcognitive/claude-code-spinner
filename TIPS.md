...
cross-checked several ways (see [Verification](#verification)). Values
...
23. Set an objective with `/goal` — Claude keeps working until it's met
...
28. `/ultrareview` runs a deep, multi-agent review of your changes
29. Try the new fullscreen renderer — flicker-free output, mouse support, auto-copy on select — `/tui fullscreen`

*(28–29 are newer tips the automated `tips.txt` reconstruction surfaced; the
numbered sections below continue from 30.)*

### Images & rewind
30. Did you know you can drag and drop image files into your terminal?
31. Paste images into Claude Code using control+v (not cmd+v!)
32. Use `ctrl+v` to paste images from your clipboard
33. Double-tap esc to rewind the conversation to a previous point in time
34. Double-tap esc to rewind the code and/or conversation to a previous point in time

### Terminal setup *(platform / wording variants)*
35. Run `/terminal-setup` to enable convenient terminal integration like Option + Enter for new line and more
36. Run `/terminal-setup` to enable convenient terminal integration like Shift + Enter for new line and more
37. Run `/terminal-setup` to enable Option+Enter for new lines
38. Run `/terminal-setup` to enable Shift+Enter for new lines
39. Press Option+Enter to send a multi-line message
40. Press Shift+Enter to send a multi-line message
41. Corrupted terminal glyphs? Disable terminal GPU acceleration in settings or run `/terminal-setup`
42. Try setting environment variable COLORTERM=truecolor for richer colors
43. Set CLAUDE_CODE_USE_POWERSHELL_TOOL=1 to enable the PowerShell tool (preview)

### IDE integration
44. Open the Command Palette (Cmd+Shift+P) and run "Shell Command: Install '`code`' command in PATH" to enable IDE integration
45. Connect Claude to your IDE — `/ide`
46. Run `/install-github-app` to tag @claude right from your Github issues and PRs
47. Run `/install-slack-app` to use Claude in Slack

### Desktop / mobile / cloud
48. Run Claude Code locally or remotely using the Claude desktop app: clau.de/desktop
49. Continue your session in Claude Code Desktop with `/desktop`
50. Working on UI? See a live preview in Claude Code Desktop — run `/desktop`
51. Working on UI? Claude Code Desktop has live preview and inline images — clau.de/desktop
52. Use Claude Design to mock up screens before you build — claude.ai/design
53. Run tasks in the cloud while you keep coding locally — clau.de/web
54. Control this session from the Claude mobile app — run `/remote-control`
55. Get pinged on your phone when long tasks finish — enable push notifications in `/config`

### Build / plugins / sharing
56. Build your AI product with Claude API. Run `/claude-api` to get started
57. Working with HTML/CSS? Install the frontend-design plugin: `/plugin install frontend-design@{marketplace}`
58. You haven't used the `{plugin}` plugin in a while. It still adds startup and context cost — review it with `/plugin`
59. You have `{count}` plugins you haven't used in a while. They still add startup and context cost — review them with `/plugin`
60. Share Claude Code and earn `{credits}` in usage credits — `/passes`
61. You have `{count}` free guest passes to share
...
3. **Ordered byte-slice.** Dump the region `New to Claude Code? Run … tipsHistory`
   in file order (`dd | strings -n 2`, **no sort**) and walk it top-to-bottom,
   joining each tip's fragments + interpolated value.
4. **Independent subagent.** A separate agent re-derived the list from the binary
   from scratch; results matched, and it surfaced the UTF-16 tails below.
5. **UTF-16LE decode.** Confirmed the plugin-disuse (#56/#57) and team-onboarding
   (#27) tails with a 16-bit decode:
   `perl -0777 -ne 'while(/((?:[\x20-\x7e]\x00){12,})/g){my $s=$1;$s=~s/\x00//g;print "$s\n"}' BIN`

6. **Automated reconstruction.** `extract.sh --tips` now reproduces this list
   directly: it walks the region in byte order, joins each tip's fragments +
   value into one sentence (Capitalized fragment = new tip; em-dash clauses
   folded back), and decodes ASCII + UTF-16. Output is `versions/<ver>/tips.txt`
   — **64 complete sentences**, every line a whole tip.

**QA anchors:** the visible `Hit shift+tab to cycle…` tip (ASCII) and a
plugin-disuse `…startup and context cost…` tip (UTF-16) are asserted on every
run (`tips_check` in `extract.sh`).

> `versions/<ver>/tips.txt` is the auto-reconstructed complete list. A few tips
> show a small gap where a purely **dynamic** value would be interpolated (a
> credit amount in #58, the editor binary in #42) — those values aren't static
> strings in the binary, so nothing is "missing" that could be recovered.
